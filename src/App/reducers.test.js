...
describe('rankingsReducer', async () => {
  ...
  const dates = Array.from(Array(6).keys()).reduce(
    (memo, i) => [...memo, addWeeks(startDate, i + 1).toDate()],
    [startDate],
  );
  ...
  try {
    test.each(testData)(
      'should produce state of next weeks rankings for $sport on $currentDate',
      async ({ sport, currentDate, nextDate }) => {
        ...
        const { entries: nextRankings } = await fetchRankings(
          sport,
          getPreviousMonday(nextDate),
        );
